...
const TreePage = ({ onNext }) => {
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col items-center justify-center p-6 overflow-hidden">
      {/* Dark Ambience Overlay - Fades out when lights turn on */}
      <motion.div 
        className="absolute inset-0 z-0"
        initial={{ backgroundColor: "#0f172a" }} // Slate-900
        animate={{ 
          backgroundColor: lightsOn ? "rgba(255, 255, 255, 0)" : "#0f172a" 
        }}
        transition={{ duration: 2, ease: "easeInOut" }}
      />

      {/* Warm Room Reflection (Radial Gradient) - Fades in */}
      <motion.div
        ...
        transition={{ duration: 2, ease: "easeInOut" }}
        style={{
          background: "radial-gradient(circle at 50% 50%, rgba(251, 191, 36, 0.2) 0%, rgba(0,0,0,0) 60%)"
        }}
      />

      <div className="z-10 w-full max-w-4xl mx-auto text-center flex flex-col items-center relative">
        <motion.div 
          className="w-32 mb-4 mx-auto"
          animate={lightsOn ? { y: [0, -10, 0] } : {}}
          transition={{ repeat: Infinity, duration: 1.5, ease: "easeInOut" }}
        >
          {/* Rabbit gets slightly brighter when lights turn on */}
          <motion.div animate={{ filter: lightsOn ? "brightness(1)" : "brightness(0.6)" }} transition={{ duration: 2 }}>
             <RabbitSVG variant={2} className="w-32 h-32" />
          </motion.div>
        </motion.div>

        {/* Tree Container with Brightness/Glow Filter */}
        <motion.div 
            className="relative mb-8 flex flex-col items-center cursor-pointer" 
            onClick={() => setLightsOn(true)}
            animate={{ filter: lightsOn ? "brightness(1) drop-shadow(0 0 20px rgba(251, 191, 36, 0.3))" : "brightness(0.4)" }}
        ...
        <AnimatePresence mode="wait">
          {!lightsOn ? (
            ...
            >
              ...
              >
                <div className="w-20 h-10 bg-slate-700 border-slate-500 rounded-full p-1 transition-all duration-300 group-hover:bg-slate-600 relative border-4 shadow-xl">
                  <motion.div 
                    layout
                    className="w-7 h-7 bg-slate-400 rounded-full shadow-md"
                  />
                </div>
                <span className="text-slate-300 font-bold text-sm tracking-widest uppercase drop-shadow-md">{D.treePage.switchLabel}</span>
              </button>
            </motion.div>
          ) : (
            <motion.div 
